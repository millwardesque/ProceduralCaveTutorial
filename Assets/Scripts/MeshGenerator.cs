...
public class MeshGenerator : MonoBehaviour {
    ...
    int GetConnectedOutlineVertex(int index) {
        List<Triangle> trianglesContainingVertex = triangleDictionary[index];
        ...
        for (int i = 0; i < trianglesContainingVertex.Count; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (vertexB != index && !checkedVertices.Contains(vertexB) && IsOutlineEdge(index, vertexB)) {
                    return vertexB;
                }
            }
        }
    ...
    bool IsOutlineEdge(int vertexA, int vertexB) {
        List<Triangle> aTriangles = triangleDictionary[vertexA];
        int sharedCount = 0;
